...
class DisplayConfig:
    ...
    def __post_init__(self):
        ...
        if self.rotation not in [0, 90, 180, 270]:
            raise ValueError(f"Invalid rotation: {self.rotation}. Must be 0, 90, 180, or 270.")
        if not 0.0 <= self.brightness <= 1.0:
            raise ValueError(f"Invalid brightness: {self.brightness}. Must be between 0.0 and 1.0.")
        if self.refresh_rate <= 0:
            raise ValueError(f"Invalid refresh rate: {self.refresh_rate}. Must be positive.")
...
class Config:
    ...
    def load(cls, config_path: Optional[str] = None) -> "Config":
        ...
        Args:
            config_path: Path to configuration file. If None, searches in standard locations.
        ...
        if config_path and Path(config_path).exists():
            try:
                ...
                config.enable_buttons = data.get('enable_buttons', config.enable_buttons)

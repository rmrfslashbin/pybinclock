...
class LEDController:
    ...
    def reset(self) -> None:
        ...
        self.field = [
            [[0, 0, 0] for x in range(self.width)] for y in range(self.height)
        ]
        self.draw()
    ...
    def toggle_pause(self) -> None:
        ...
        self.set_status(
            "paused",
            (
                self.config.colors.status_error
                if self.paused
        ...
        with LEDController(config) as controller:
            ...
            while True:
                if controller.exit:
                    ...
                    controller.write_text("Shutting down!", config.colors.status_okay)
                    break
